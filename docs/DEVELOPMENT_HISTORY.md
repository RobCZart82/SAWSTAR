...
Archived pre-RC2 README. Status/version/count statements below are historical;
use the root README and CHANGELOG for the current candidate.
...
with a custom [7-Saw unison layer](SEVEN_SAW.md).

## 1.0.0 release preparation

The source is now a **1.0.0 pre-release candidate**, not a published v1.0.0.
See [candidate notes](RELEASE_NOTES_1.0.0.md) and
[installation instructions](INSTALLATION.md).

### User manuals

- [English PDF](manuals/SAWSTAR-User-Manual-EN.pdf)
- [Magyar használati útmutató (PDF)](manuals/SAWSTAR-User-Manual-HU.pdf)
...
![SAWSTAR Master GUI: MAIN, ADVANCED and PRESETS](reference/SAWSTAR_Master_GUI_REAPER.png)
...
[Master GUI details](MASTER_GUI.md) · [Earlier design concept](reference/SAWSTAR_GUI_Concept.png)

### About

![SAWSTAR About window](reference/SAWSTAR_About_REAPER.png)

*Actual About window from the same development build, with the GYR monogram and both mottos.*

## Built-in sounds

The [embedded Factory library](FACTORY_LIBRARY.md) contains **24 sounds**:
...
The concept-based [preset browser](USER_PRESETS.md) combines factory and user
...
The MAIN AMP ENV panel includes a green [PRE-FX oscilloscope](REALTIME_SCOPE.md),
with a fixed 32 ms view and no additional sound parameters.

## Master WIDE

The OUTPUT panel now offers [master stereo width](MASTER_WIDTH.md), with
...
See [GUI implementation and validation](GUI_IMPLEMENTATION.md) for controls,
known differences from the artwork and host-check coverage.

The September 10 [audit fixes](AUDIT_FIXES.md) protect concurrent preset saves,
...
Repeated MIDI notes now use [paired press/release handling](OVERLAPPING_NOTES.md),
including ARP bypass and all three voice modes.

The [preset reliability and GUI plan](PRESET_GUI_PLAN.md) covers safe favorites,
...
see [GUI implementation](GUI_IMPLEMENTATION.md) for actual behavior.
...
- [First release plan](FIRST_RELEASE_PLAN.md)
- [Latest Templates / host QA](TEMPLATES_HOST_QA.md)
- [Earlier engine/GUI QA](PRE_RELEASE_QA.md)
- [Installation and release preparation](INSTALLATION.md)
- [Windows manual checklist](WINDOWS_ACCEPTANCE.md)

See [the embedded library](FACTORY_LIBRARY.md) for Templates and the
...
No SDK download scripts run automatically. See [BUILDING.md](BUILDING.md).
...
- [Concept](CONCEPT.md)
- [Architecture and real-time boundaries](ARCHITECTURE.md)
- [Parameter and state contract](PARAMETERS.md)
- [GUI reference slot](reference/README.md)
- [Dependency versions and licensing](THIRD_PARTY.md)
...
and keyboard tracking. See [filter modulation](FILTER_MODULATION.md).

Pitch bend and CC1 now work per MIDI channel, with shared on-screen wheels
and ADVANCED depth controls. See [performance controls](PERFORMANCE.md).

The [four-source mixer and output calibration](SOURCE_MIXER.md) add an
...
[Filter Drive and four filter modes](FILTER_CHARACTER.md) now provide
LP12, LP24, HP12 and BP12. Raise Filter Mix to engage them; existing projects
retain undriven LP12 until changed.

[Oscillator waveform previews and LFO routing](LFO_WAVEFORMS.md) add
...
Stereo chorus is available on ADVANCED (Off by default). See [chorus](CHORUS.md).

Stereo/ping-pong delay is available on ADVANCED (Off by default). See [delay](DELAY.md).

Stereo reverb is available in ADVANCED → REVERB (Off by default). See [reverb](REVERB.md).

Mono/Legato and Glide are available on ADVANCED. See [performance modes](MONO_GLIDE.md).
...
default. See [Modulation](MODULATION.md) for controls and compatibility.
...
[Arpeggiator](ARPEGGIATOR.md) for playing and transport behavior.
