...
inline void BuildLayout(IGraphics* g,float& guiScale,int& page,int& lfoPage,int& fxPage,const int& preset,std::function<void(int)> load,const std::atomic<float>& peakL,const std::atomic<float>& peakR,const std::atomic<float>& cpu,const std::atomic<int>& rate,const std::atomic<int>& voices,UserPresetSelection& user,std::function<Snapshot()> current,std::function<void(const Snapshot&)> apply){
 ...
 bool brandFont=g->LoadFont("SAWSTAR-Orbitron",AboutFont,sizeof(AboutFont));g->AttachControl(new BrandWordmark(IRECT(20,15,282,63),brandFont));text(IRECT(287,20,422,38),"Simple Synth",13);text(IRECT(287,38,422,56),"B i g  S o u n d",13);
 text(IRECT(20,62,430,77),SAWSTAR_HEADER_VERSION,10);
 ...
 g->AttachControl(new Keyboard(IRECT(115,654,1258,715),36,96,false,IColor(255,202,209,210),IColor(255,19,24,27),Blue,PanelColor,Text));g->AttachControl(new Status(IRECT(20,729,830,750),cpu,rate,voices),9101);
 g->AttachControl(new ITextControl(IRECT(830,729,1260,750),Motto,IText(12,Text).WithFont("SAWSTAR-Bold").WithAlign(EAlign::Far)));
