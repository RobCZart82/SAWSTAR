...
inline void BuildLayout(IGraphics* g,int& page,int& lfoPage,int& fxPage,const int& preset,std::function<void(int)> load){
 ...
 auto text=[&](IRECT r,const char* label,int size,const char* group=""){g->AttachControl(new ITextControl(r,label,IText(size,Text).WithAlign(EAlign::Near)),kNoTag,group);};
 auto section=[&](IRECT r,const char* name,const char* group,IColor color=Blue,bool tint=false){g->AttachControl(new Section(r,name,color,tint),kNoTag,group);};
 auto knob=[&](float x,float y,float w,int id,const char* label,const char* group){g->AttachControl(new Knob(IRECT(x,y,x+w,y+79),id,label),kNoTag,group);};
 auto menu=[&](IRECT r,int id,const char* label,const char* group){g->AttachControl(new IVMenuButtonControl(r,id,label,Style()),kNoTag,group);};
 auto fader=[&](IRECT r,int id,const char* label,const char* group,bool horizontal=false){g->AttachControl(new Fader(r,id,label,horizontal?EDirection::Horizontal:EDirection::Vertical),kNoTag,group);};
 ...
 for(int osc=0;osc<2;++osc){float y=126+osc*173;g->AttachControl(new WaveformControl(IRECT(23,y,145,y+146),33+osc,osc?"OSC 2":"OSC 1"),kNoTag,"main");int ids[4]={osc?24:30,osc?27:5,osc?28:6,osc?29:7};const char* labels[]={"OCT","DETUNE","UNISON","WIDTH"};for(int j=0;j<4;++j)knob(150+(j%2)*66,y+(j/2)*80,63,ids[j],labels[j],"main");}
 ...
 const char* sources[]={"OSC1","OSC2","SUB","NOISE"};for(int i=0;i<4;++i)fader(IRECT(308+i*37,126,344+i*37,457),20+i,sources[i],"main");fader(IRECT(312,480,452,553),63,"NOISE COLOR","main",true);g->AttachControl(new NoiseSelector(IRECT(311,586,453,621)),kNoTag,"main");
 g->AttachControl(new Curve(IRECT(482,128,638,259),{8,32},false),kNoTag,"main");knob(478,280,55,8,"CUTOFF","main");knob(533,280,55,9,"RES","main");knob(588,280,55,31,"DRIVE","main");fader(IRECT(484,386,636,449),10,"FILTER MIX","main",true);fader(IRECT(484,489,636,554),12,"KEY TRACK","main",true);menu(IRECT(483,582,637,623),32,"","main");
 const char* adsr[]={"A","D","S","R"};for(int e=0;e<2;++e){float x=e?834:656;g->AttachControl(new Curve(IRECT(x+10,128,x+160,259),e?std::initializer_list<int>{1,2,3,4}:std::initializer_list<int>{13,14,15,16},true),kNoTag,"main");for(int j=0;j<4;++j)knob(x+3+j*41,280,40,(e?1:13)+j,adsr[j],"main");if(!e)knob(x+42,464,85,11,"AMOUNT","main");}
 ...
 fader(IRECT(1156,126,1238,421),0,"VOLUME","main");knob(1147,480,100,19,"BOOST dB","main");
 ...
 for(int bank=0;bank<2;++bank){g->AttachControl(new PageButton(IRECT(674+bank*97,89,766+bank*97,116),bank?"LFO 2":"LFO 1",bank,lfoPage,[&lfoPage,&page,select,bank]{lfoPage=bank;select(page);}),kNoTag,"advanced");const char* group=bank?"lfo2":"lfo1";int base=bank?64:35;
 ...
 for(int row=0;row<4;++row){float y=132+row*67;int id=71+row*3;menu(IRECT(899,y,1006,y+51),id,row==0?"SOURCE":"","advanced");menu(IRECT(1014,y,1163,y+51),id+1,row==0?"DESTINATION":"","advanced");knob(1173,y-7,83,id+2,row==0?"AMOUNT":"","advanced");}
 section(IRECT(12,433,1268,636),"EFFECT DETAILS","advanced");const char* effects[]={"CHORUS","DELAY","REVERB"};for(int i=0;i<3;++i)g->AttachControl(new PageButton(IRECT(24+i*112,478,129+i*112,515),effects[i],i,fxPage,[&fxPage,&page,select,i]{fxPage=i;select(page);}),kNoTag,"advanced");text(IRECT(24,539,350,601),"Mix and enable are also on MAIN",12,"advanced");
 ...
 section(IRECT(12,82,570,636),"FACTORY LIBRARY","presets");section(IRECT(580,82,1268,636),"PRESET INFO / LEARNING","presets");for(int i=0;i<int(FactoryPresets().size());++i)g->AttachControl(new LibraryRow(IRECT(24,129+i*53,558,177+i*53),i,preset,load),kNoTag,"presets");g->AttachControl(new LibraryInfo(IRECT(604,134,1245,551),preset),kNoTag,"presets");text(IRECT(604,577,1245,619),"Save your own sounds using the host preset menu.",14,"presets");
 ...
 g->AttachControl(new Keyboard(IRECT(115,654,1258,715),36,96,false,IColor(255,181,187,187),IColor(255,19,24,27),Blue,PanelColor,Text));text(IRECT(20,729,1260,750),"SAWSTAR  |  16 VOICES  |  KEYBOARD / WHEELS: MIDI CH 1",11);
 select(page);
}
}
