...
public:
 ...
 void Draw(IGraphics& g)override{g.FillRect(IColor(190,0,0,0),mRECT);auto b=Box();g.FillRoundRect(PanelColor,b,7);g.DrawRoundRect(Border,b,7);g.DrawText(IText(22,Text),title_.c_str(),b.GetFromTop(59));
  const auto style=IText(14,Text).WithAlign(EAlign::Near);
  std::string text=message_;float y=b.T+66;
  while(!text.empty()){
   // Fit actual glyph widths; character counts break short sentences unnecessarily.
   size_t n=text.find('\n');if(n==std::string::npos)n=text.size();
   IRECT measured;g.MeasureText(style,text.substr(0,n).c_str(),measured);
   while(measured.W()>b.W()-48&&n>0){
    auto space=text.rfind(' ',n-1);if(space==std::string::npos||space==0)break;
    n=space;g.MeasureText(style,text.substr(0,n).c_str(),measured);
   }
   g.DrawText(style,text.substr(0,n).c_str(),IRECT(b.L+24,y,b.R-24,y+23));
   text.erase(0,n);if(!text.empty()&&(text.front()==' '||text.front()=='\n'))text.erase(0,1);
   y+=24;
  }
