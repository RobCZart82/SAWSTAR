...
public:
 ...
 if(action==0){if(!ValidPresetName(state_.name))throw std::runtime_error("Use a name of 1-80 characters without path symbols.");auto root=UserPresetFolder();if(root.empty())throw std::runtime_error("User preset folder is unavailable.");auto p=root/fs::u8path(state_.name+".sawstar");auto values=current_();SaveUserPreset(p,values);state_.path=p;state_.saved=values;state_.active=true;state_.status="Saved user preset";}
