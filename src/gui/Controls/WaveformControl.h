...
public:
 WaveformControl(const iplug::igraphics::IRECT& r,int param,const char* title):IControl(r,param),title_(title){for(auto n:names_)menu_.AddItem(n);}
 ...
 void OnPopupMenuSelection(iplug::igraphics::IPopupMenu* menu,int)override{
  if(menu&&menu->GetChosenItemIdx()>=0){SetValue(menu->GetChosenItemIdx()/3.);SetDirty(true);}
 }
private:
