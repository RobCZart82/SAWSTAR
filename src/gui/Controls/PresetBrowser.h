...
class PresetBrowser final:public IControl {
 ...
 void Info(IGraphics& g){const auto* e=Selected();if(!e)return;TextLine(g,e->name,IRECT(651,140,1038,177),22);
  ...
  for(int i=0;i<5;++i){IRECT r(651+i*82,399,723+i*82,450);g.FillRoundRect(IColor(255,10,18,24),r,3);const IColor colors[]={IColor(255,70,206,237),IColor(255,144,173,246),IColor(255,220,179,86),IColor(255,84,213,186),IColor(255,178,143,231)};const auto color=colors[i];g.DrawRoundRect(color,r,3);g.DrawText(IText(11,color),blocks[i],r.GetFromTop(19));auto plot=IRECT(r.L+7,r.T+23,r.R-7,r.B-6);
   if(i==0||i==2){float px=plot.L,py=plot.MH();for(int j=0;j<=30;++j){float t=j/30.f,y=0;if(i==0){int wave=int(std::lround(v[33]));y=wave==0?2*t-1:wave==1?(t<.5f?1.f:-1.f):wave==2?1-4*std::abs(t-.5f):std::sin(t*6.2831853f);}else{float cutoff=float(Normalize(kParameters[8],v[8]));float low=1/(1+std::exp((t-cutoff)*14));int mode=int(v[32]);y=2*(mode<2?low:mode==2?1-low:4*low*(1-low))-1;}float x=plot.L+t*plot.W(),sy=plot.MH()-y*plot.H()*.45f;if(j)g.DrawLine(color,px,py,x,sy);px=x;py=sy;}}
   else if(i==1){for(int j=0;j<4;++j){float x=plot.L+j*plot.W()/4;g.FillRect(color,IRECT(x,plot.B-float(v[20+j]/100)*plot.H(),x+6,plot.B));}}
   else if(i==3){float total=float(v[1]+v[2]+v[4])+250.f;float a=float(v[1])/total,d=float(v[2])/total,release=float(v[4])/total;float xs[]={0,a,a+d,1-release,1},ys[]={0,1,float(v[3]),float(v[3]),0};for(int j=1;j<5;++j)g.DrawLine(color,plot.L+xs[j-1]*plot.W(),plot.B-ys[j-1]*plot.H(),plot.L+xs[j]*plot.W(),plot.B-ys[j]*plot.H());}
   else{for(int j=0;j<3;++j){int id=j==0?42:j==1?46:54;g.FillCircle(v[id]>.5&&v[id+1]>0?color:Border,plot.L+10+j*19,plot.MH(),5);}}
  }
...
public:
 ...
 void Draw(IGraphics& g)override{
  ...
  g.FillRoundRect(DisplayColor,Search(),3);g.DrawRoundRect(Border,Search(),3);TextLine(g,query_.empty()?"Search presets...":query_,Search().GetHPadded(-8),13);
