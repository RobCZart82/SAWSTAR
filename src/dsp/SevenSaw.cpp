  ...
  for(size_t i=0;i<saws_.size();++i) {
    ...
    for(int w=1;w<4;++w)if(waveWeights_[w]>0){auto& o=alternatives_[w-1][i];
      ...
      if(w==2){
        ...
        const float increment=frequency/rate_;
        triangleHistory_[i]=increment*(value/.707f)+(1-increment)*triangleHistory_[i];
        value=4*triangleHistory_[i];
      }
      sample+=value*waveWeights_[w];}
