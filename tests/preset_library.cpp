...
int main(){using namespace sawstar;auto root=fs::temp_directory_path()/("sawstar-library-"+std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));try{
 ...
 PresetLibrary lib(dest);check(lib.Filter("All","").size()==FactoryPresets().size()+30&&lib.Filter("User","").size()==30);check(lib.Filter("User","SOUND 29").size()==1);check(lib.Filter("Lead","").size()==3&&lib.Filter("Templates","").size()==5);check(lib.Filter("FX","").empty());
