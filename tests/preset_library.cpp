...
int main(){using namespace sawstar;auto root=fs::temp_directory_path()/("sawstar-library-"+std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));try{
 ...
 std::vector<fs::path> files;for(int i=0;i<30;++i){auto p=source/("Sound "+std::to_string(i)+".sawstar");SaveUserPreset(p,values);files.push_back(p);}
 ...
 PresetLibrary lib(dest);check(lib.Filter("All","").size()==38&&lib.Filter("User","").size()==30);check(lib.Filter("User","SOUND 29").size()==1);check(lib.Filter("Lead","").size()==2&&lib.Filter("Init","").size()==1);check(lib.Filter("FX","").empty());
 lib.ToggleFavorite("user:Sound 0.sawstar");lib.ToggleFavorite("factory:init");PresetLibrary restored(dest);check(restored.Filter("Favorites","").size()==2);restored.MoveFavorite("user:Sound 0.sawstar","user:Renamed.sawstar");RenameUserPreset(dest/"Sound 0.sawstar","Renamed");restored.Refresh();check(restored.Filter("Favorites","").size()==2);ArchiveUserPreset(dest/"Renamed.sawstar");restored.Refresh();check(restored.Filter("Favorites","").size()==1);check(ImportPresets({},dest).imported==0);
