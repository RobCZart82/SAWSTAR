...
int main(){for(float sr:{44100.f,48000.f,96000.f}){
 ...
 auto aStorage=std::make_unique<sawstar::Synth>();auto& a=*aStorage;auto bStorage=std::make_unique<sawstar::Synth>();auto& b=*bStorage;setup(a,sr);setup(b,sr);b.SetLfo2(19,0,3,2,true,5,145,true);
 ...
 for(int source=1;source<=5;++source)for(int target=0;target<5;++target){
  auto plainStorage=std::make_unique<sawstar::Synth>();auto& plain=*plainStorage;auto modStorage=std::make_unique<sawstar::Synth>();auto& mod=*modStorage;setup(plain,sr);setup(mod,sr);
 ...
 for(int target=0;target<4;++target){auto plainStorage=std::make_unique<sawstar::Synth>();auto& plain=*plainStorage;auto modStorage=std::make_unique<sawstar::Synth>();auto& mod=*modStorage;setup(plain,sr);setup(mod,sr);
 ...
 auto plainStorage=std::make_unique<sawstar::Synth>();auto& plain=*plainStorage;auto pressureStorage=std::make_unique<sawstar::Synth>();auto& pressure=*pressureStorage;setup(plain,sr);setup(pressure,sr);pressure.SetModulation(0,5,1,50);
 ...
 auto stressStorage=std::make_unique<sawstar::Synth>();auto& stress=*stressStorage;setup(stress,sr);stress.SetLfo(5,100,2,0,false,2,120,true);stress.SetLfo2(3,100,0,1,true,3,140,true);
