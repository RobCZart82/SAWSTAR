...
int main(){for(float sr:{44100.f,48000.f,96000.f}){
 ...
 auto aStorage=std::make_unique<sawstar::Synth>();auto& a=*aStorage;auto bStorage=std::make_unique<sawstar::Synth>();auto& b=*bStorage;for(auto s:{&a,&b}){s->Reset(sr);s->SetParameters(-12,1,1,1,20);s->Midi(0x90,60,100);}
 ...
 for(int target=0;target<4;++target){auto plainStorage=std::make_unique<sawstar::Synth>();auto& plain=*plainStorage;auto modStorage=std::make_unique<sawstar::Synth>();auto& mod=*modStorage;
 ...
 auto cStorage=std::make_unique<sawstar::Synth>();auto& c=*cStorage;auto dStorage=std::make_unique<sawstar::Synth>();auto& d=*dStorage;for(auto s:{&c,&d}){s->Reset(sr);s->SetMixer(0,100,0,0,0,-1,0,0);s->SetParameters(0,1,1,1,20);s->Midi(0x90,60,127);}
